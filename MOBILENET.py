...
device = 'cuda:0'
train_loader, test_loader = ut.load_dataset(model='MOBILENET')
...
for epoch in range(num_epochs):
    ...
    if test_acc_max < test_acc:
        ...
        }, 'checkpoint/best_model_mobilenet_checkpoint.pth')


    TP, TN, FP, FN, accuracy = ut.evalauation(torch.cat(test_output, dim=0), torch.cat(test_label, dim=0), epoch = epoch + 1, title='MOBILENET')
    ut.plot_accuracy_values(train_accuracy, test_accuracy, 'MOBILENET')
    print('TP',TP)
    print('TN',TN)
    print('FP',FP)
    print('FN',FN)
